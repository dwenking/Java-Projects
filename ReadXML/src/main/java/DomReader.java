...
{
	//自上而下进行访问
    public static Double recursiveTraverse()
    {
    	Double res=0.;
    	...
    	try 
    	{
            ...
            Document document = db.parse("1.xml");
                    ...
                    if (user.getNodeType() == Node.ELEMENT_NODE)
                    {
                         ...
                         for (int k = 0; k < metaList.getLength(); k++) //7
                         {
                        	...
                        	if (meta.getNodeType() == Node.ELEMENT_NODE)
                        	{
                        		...
                        		if(metaList.item(k).getNodeName() =="name") {
                        			stu.add(metaList.item(k).getTextContent());
                        		}
                        		if(metaList.item(k).getNodeName() =="score") {
                        			total_scores+=Double.parseDouble(metaList.item(k).getTextContent());
                        		}
                        	}                                                              
                         }                    
                         //System.out.println();
